...
export async function POST(request: NextRequest) {
    try {
        ...
        const productsResponse = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://10.241.197.145:8080'}/api/v1/cms/parking-membership-product`, {
            method: 'GET',
            headers: {
        ...
        const existingMembershipsResponse = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://10.241.197.145:8080'}/api/v1/cms/parking-membership`, {
            method: 'GET',
            headers: {
                'Authorization': `Bearer ${token}`,
            },
        ...
        for (let i = 0; i < transformedData.length; i++) {
            try {
                const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://10.241.197.145:8080'}/api/v1/cms/parking-membership`, {
                    method: 'POST',
                    headers: {
